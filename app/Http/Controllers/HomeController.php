    ...
    public function registerStepOptions(Request $request) {
		...
		if ($request->session()->has('registration')) {
			...
			switch($registrationoptions['option']) {
				case 1: $cost=$options['option1price']; break;
				case 2: $cost=$options['option2price']; break;
				case 3: $cost=$options['option3price']; break;
				case 4: 
			...
			if(!empty($jerseys)) {
				$cost += (sizeof($jerseys)-1)*$options['jersey'];
			}
